...
def read_pools(pool_class, file, rxn_time=None, use_k=False):
    ...
    if use_k and rxn_time:
        fitnesses = [
            -np.log(1 - x) / rxn_time if x < 1 else float("inf") for x in outputs
        ]
    else:
        fitnesses = outputs
    ...
    if any(np.isnan(fitnesses)):  # hasn't been filled in
        raise ValueError(
            "The file does not have the expected output values for this generation!"
        )
...
def read_singles(mode, singles_file, rxn_time=None, use_k=False):
    ...
    if mode == "sim":  # singles k's are in file
        ...
        if use_k and rxn_time:
            singles_outputs = [
                -np.log(1 - x) / rxn_time if x < 1 else float("inf")
                for x in singles_yields
            ]
        else:
...
def deconvolute(frac_top, final_size, target_metric, num_top, pop, verbose):
    ...
    for key in intersection_dict:
        intersection_dict[key] = sum(intersection_dict[key]) / len(
            intersection_dict[key]
        )

    # Sort intersections by score
    sorted_intersections = sorted(
        intersection_dict.items(), key=lambda x: x[1], reverse=True
    )
    ...
    num_ties = sum(
        1
        for entry in sorted_intersections[num_top:]
...
def pre_config_oneshot(
    ...
    **kwargs,
    ...
    if mode in ["sim", "rec"]:
        toolbox.register(
            "start", init_covering, creator.Pool, N, pool_size, num_meet, num_redun
        )
    ...
    toolbox.register(
        "deconvolute", deconvolute, frac_top, final_size, target_metric, num_top
    )
...
def run_oneshot(
    ...
    if "frac_top" not in config:
        config["frac_top"] = 1.0
    if "tiebreak_round" not in config:
        config["tiebreak_round"] = True
    if "score_mode" not in config:
        # for rec mode we don't ask user for score mode
        config["score_mode"] = "avg"

    # Configure toolbox
    toolbox = pre_config_oneshot(
        N,
        config["pool_size"],
        config["num_meet"],
        config["num_redun"],
    ...
    if mode == "sim":
        ...
        if len(pop[0]) == config["final_size"]:
            ...
            if config["target_metric"] == "coop":
                sorted_pools = sorted(
                    pop, key=lambda x: x.fitness.values[1], reverse=True
                )
                sorted_pop_with_scores = [
                    (ind, ind.fitness.values[1]) for ind in sorted_pools
                ]
            else:  # output
                sorted_pools = sorted(
                    pop, key=lambda x: x.fitness.values[0], reverse=True
                )
                sorted_pop_with_scores = [
                    (ind, ind.fitness.values[0]) for ind in sorted_pools
                ]
    ...
    try:
        ...
        # Score pools
        pop, fitnesses, coop_scores = score_pools(
            pop, mode, toolbox, singles_ks, pool_parallel
        )
        num_pools = len(pop)
        ...
        if config.get(
            "tiebreak_round", False
        ):  # returns true if tiebreak_round exists in yaml, else returns False
...
def real_oneshot(online=False, outfile=None, **kwargs):
    ...
    if "config_dict" in kwargs:
        ...
        if (
            "score_mode" not in kwargs["config_dict"]
        ):  # for rec mode we don't ask user for score mode
    ...
    if kwargs["mode"] == "rec":
        ...
        df = pd.DataFrame(
            {"pool": pop_out, "output": None}  # Column for user to fill in
        )
