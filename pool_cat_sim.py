...
class DimerReaction(Reaction):
    ...
    def __init__(self, cats, rxn_time=0.05):
        ...
        if self.cats:
            self.cat_species_concs = (
                self.equil_cats()
            )  # get concentrations of all active cats after equilibrating
    ...
    @jit(
        float64[:](
            float64, float64[:], int64[:], int64[:], float64[:], float64[:], int64[:]
        ),
        nopython=True,
    )
    ...
    def equil_cats_numba(self):
        ...
        def set_up_eqm(irrev_k):
            ...
            all_coop_pairs = set(DimerReaction.pos_coop_dict.keys()) | set(
                DimerReaction.neg_coop_dict.keys()
            )
            ...
            if DimerReaction.neg_coop_dict:
                for k, v in DimerReaction.neg_coop_dict.items():
                    if k[0] in self.cats and k[1] in self.cats and k[0] != k[1]:
                        reactions.append(
                            (k, (DimerReaction.neg_strength * 0.1, irrev_k))
                        )
            ...
            sol = solve_ivp(
                numba_odes, [0, 200], y0, method="RK45", rtol=1e-6, atol=1e-12
            )
        ...
        return pd.DataFrame.from_dict(
            conc_dict, orient="index", columns=["concentration"]
        )
    ...
    def equil_cats(self):
        ...
        def set_up_eqm(irrev_k):
            ...
            params = {"reactions": reactions, "index_map": index_map}
            # print(f"Solving ODE system with initial y0: {y0} and params: {params}")

            sol = solve_ivp(
                odes, [0, 200], y0, args=(params,), method="RK45", rtol=1e-6, atol=1e-12
            )
    ...
    def equil_cats_cps(self):
        ...
        def set_up_eqm(irrev_k):
            ...
            for idx in self.cats:
                ...
                cps.set_species(
                    f"c{idx}", initial_concentration=1 / self.num_cats
                )  # if we have two cats then half each

            if DimerReaction.neg_coop_dict:
                # stick all the negatively cooperative catalysts together
                for k in DimerReaction.neg_coop_dict:
                    if (k[0] in self.cats) and (
                        k[1] in self.cats
                    ):  # a cat pair applies here!
                        rxn_name = f"c{k[0]}-c{k[1]}b"
                        cps.add_reaction(
                            rxn_name, f"c{k[0]} + c{k[1]} = c{k[0]}_c{k[1]}"
                        )  # b for bind
                        cps.set_species(f"c{k[0]}_c{k[1]}", initial_concentration=0)
                        cps.set_reaction_parameters(
                            f"({rxn_name}).k2", value=irrev_k
                        )  # irreversible for now
                        cps.set_reaction_parameters(
                            f"({rxn_name}).k1", value=DimerReaction.neg_strength * 0.1
                        )  # irreversible for now
        ...
        for cat, row in self.cat_species_concs.iterrows():
            ...
            if concentration > 1e-10:  # Ignore very small concentrations
                ...
                if len(cat_species) == 1:
                    ...
                    k_set_composite = sum(
                        [DimerReaction.cat_rate_dict[cpt] for cpt in cat_species]
                    )
                    sorted_species = tuple(
                        sorted(cat_species)
                    )  # Ensure consistent ordering
                    if sorted_species in DimerReaction.pos_coop_dict:
                        k_set = (
                            k_set_composite
                            * DimerReaction.pos_coop_dict[sorted_species]
                        )
                    elif sorted_species in DimerReaction.neg_coop_dict:
                        k_set = (
                            k_set_composite
                            * DimerReaction.neg_coop_dict[sorted_species]
                        )
                    else:
                        raise KeyError(
                            f"Catalytic species {sorted_species} not found in coop dict"
                        )
    ...
    def run_rxn(self):
        ...
        try:
            sol = solve_ivp(
                rxn, [0, self.rxn_time], y0, args=(self.k_eff,), method="LSODA"
            )
            if sol.success:
    ...
    @classmethod
    def catalyst_generator(
        cls, out, N, seed=50, mean_rate=10, sd_rate=10, rate_outlier_prob=0.1, **kwargs
    ):
        """
    ...
    def coop_generator(cls, out, **kwargs):
        ...
        def gen_pos_coop_mat(N, p_pos=0.01, range_pos=5, **kwargs):
            ...
            matrix[indices[0][outlier_indices], indices[1][outlier_indices]] = (
                rng.choice(np.arange(2, range_pos + 1), num_outliers, replace=True)
            )
            ...
            for i, j in zip(indices[0][outlier_indices], indices[1][outlier_indices]):
                ...
                pos_coop_dict_loc[(int(i), int(j))] = float(matrix[i, j])
        ...
        def gen_neg_coop_mat(N, p_neg=0.05, range_neg=0.3, **kwargs):
            ...
            # print(outlier_indices)
            choices = np.array(
                [i for i in np.arange(len(indices[0])) if i not in outlier_indices]
            )
            ...
            matrix[indices[0][neg_outlier_indices], indices[1][neg_outlier_indices]] = (
                rng.uniform(0, range_neg, num_outliers)
            )
            np.fill_diagonal(matrix, 1)

            neg_coop_dict_loc = {}
            for i, j in zip(
                indices[0][neg_outlier_indices], indices[1][neg_outlier_indices]
            ):
                if i != j:
                    neg_coop_dict_loc[(int(i), int(j))] = float(matrix[i, j])
...
class MultistepReaction(Reaction):
    ...
    def calculate_k_eff(self):
        ...
        for cat in self.cat_species_concs.keys():  # cat is the specific index
            for i in range(MultistepReaction.num_steps):
                self.k_eff[i] += (
                    self.cat_species_concs[cat]
                    * MultistepReaction.cat_rate_dict[cat][i]
                    * MultistepReaction.coop_dict[cat][i]
                )
    ...
    def catalyst_generator(
        ...
        outlier_indices = rng.choice(len(rates), num_outliers, replace=False)
        outliers = rng.normal(
            loc=mean_rate * 2, scale=sd_rate, size=(num_outliers, cls.num_steps)
        )
        # more knobs to turn here, potentially
        rates[outlier_indices] = outliers
    ...
    def coop_generator(cls, N, out, seed, coop_means, coop_corrs, coop_vars, **kwargs):
        ...
        # no outliers for now
        cls.coop_dict = {
            index: tuple(affinity) for index, affinity in zip(range(N), affinities)
        }
...
def gen_landscape(config_file=None, config_dict=None, out=None):
    ...
    for i in range(num_landscapes):
        ...
        rxn_class.catalyst_generator(
            seed=rng.integers(0, 1e10), out=out_i, **config["simul"]
        )
        rxn_class.coop_generator(
            seed=rng.integers(0, 1e10), out=out_i, **config["simul"]
        )
...
def set_landscape(rate_file, coop_file, **kwargs):
    ...
    if Reaction.coop_mech == "dimer":
        ...
        rxn_class.cat_rate_dict, rxn_class.pos_coop_dict, rxn_class.neg_coop_dict = (
            cat_rate_dict,
            pos_coop_dict,
            neg_coop_dict,
        )
